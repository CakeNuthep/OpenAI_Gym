...
class Buffer:
    ...
    def __init__(self, observation_dimensions, size, gamma=0.99, lam=0.95):
        ...
        self.observation_buffer = np.zeros(
            (size,ROWS,COLS,REM_STEP), dtype=np.float32
        )
...
def mlp(x, sizes, activation=tf.tanh, output_activation=None):
    ...
    for size in sizes[:-1]:
        x = layers.Conv2D(64, 5, strides=(3, 3),padding="valid", activation=activation, data_format="channels_last")(x)
...
buffer = Buffer(state_size, steps_per_epoch)

# Initialize the actor and the critic as keras models
observation_input = keras.Input(shape=state_size, dtype=tf.float32)
logits = mlp(observation_input, list(hidden_sizes) + [num_actions], 'relu', None)
actor = keras.Model(inputs=observation_input, outputs=logits)
value = tf.squeeze(
    mlp(observation_input, list(hidden_sizes) + [1], 'relu', None), axis=1
)
...
observation, episode_return, episode_length = reset(), 0, 0
...
for epoch in range(epochs):
    ...
    for t in range(steps_per_epoch):
        ...
        observation_new, reward, done, _ = step(action[0].numpy())
    ...
    print("Epoch: {}. Mean Return:{}. Mean Legnth:{} ".format(epoch+1, sum_return/num_episodes, sum_length/num_episodes))